...
h_b_size = 50
e_c_size = int(h_b_size**0.25)
ip_cul=tf.feature_column.categorical_column_with_hash_bucket(key="ip",hash_bucket_size=h_b_size)
...
classifier = tf.estimator.DNNClassifier(model_dir=DIR,feature_columns=my_feature_columns,hidden_units=[100, 100],n_classes=2)
...
#print (train_feature)
#print (test_label)
classifier.train(input_fn=lambda:pipe.train_input_fn(train_feature, train_label,batch_size=200) ,steps=15000)

#classifier.export_savedmodel("./MODELX/",tf.estimator.export.build_parsing_serving_input_receiver_fn)

# Evaluate the model.

eval_result = classifier.evaluate(input_fn=lambda:pipe.eval_input_fn(test_feature, test_label, batch_size=50))
